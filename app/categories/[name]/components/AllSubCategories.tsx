...
export default function AllSubCategories({categories}: AllSubCategoriesProps) {
  return (
    <main className="container flex flex-col gap-6">
       ...
       <>
        <section key={index} className={`bg-categoryBg py-[32px] px-[12px] rounded-lg `}>
          ...
          <section>
            ...
            <div className={`grid gap-5  grid-cols-2 lg:grid-cols-2 xl:grid-cols-5  md:grid-cols-1 lg:grid-cols-1`}>
           {cat.products.map((item,index)=>{
            return  <Link
               key={index}
                 href={`/categories/${encodeURIComponent(cat.name)}/${`${item.name}`}`} 

                // href={`/search?name=${encodeURIComponent(item.title)}`}
              > <div  className={`border cursor-pointer border-description rounded-[12px]   md:grid-cols-5]`}>
      ...
      <button className="text-sm mt-1 inline-flex items-center gap-2 text-description bg-white  hover:bg-white rounded py-1 px-2  w-fit "> <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 13 12" fill="none">
      ...
      >
        ...
        <path d="M7.05566 1C4.01816 1 1.55566 3.4625 1.55566 6.5C1.55566 12 8.05566 17 11.5557 18.163C15.0557 17 21.5557 12 21.5557 6.5C21.5557 3.4625 19.0932 1 16.0557 1C14.1957 1 12.5507 1.9235 11.5557 3.337C11.0484 2.61469 10.3746 2.0252 9.59134 1.61841C8.80805 1.21162 7.93829 0.999502 7.05566 1Z" stroke="#172556" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
               ...
               <div className="px-3 py-4 flex flex-col gap-4 md:col-span-4 w-full "> <div className={`flex flex-col  gap-3  `}> 
      ...
      <h4 className={`text-[21px] font-poppins font-medium leading-[110%] text-name mb-1 line-clamp-2 min-h-[50px] `}>
      {item.name}
      </h4>
