...
const Page = () => {
  ...
  const [showMenu, setShowMenu] = useState(true); // mobile: show sidebar first
    ...
    const menuItems = [
      { id: 'profile', label: 'Profile Settings', icon: <User size={18} />,component:     <Profile   /> },
      { id: 'notifications', label: 'Notification Preferences', icon: <Bell size={18} />, Component:<Notifications label={undefined} /> },
  ...
  const renderContent = () => {
    switch (activePage) {
      case "profile":
        return (
         <Profile label={activeComponent?.label}  />
        );
      default:
        return (
          <div >
